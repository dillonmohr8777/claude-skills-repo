...
# The reference deck measures 12191969 EMU wide, 31 short of a nominal 13.333in
# canvas (12192000). A deck rebuilt at exact nominal size is the same 16:9 slide
# and must not fail, so geometry is compared with tolerance rather than equality.
EXPECTED_GEOMETRY = (12191969, 6858000)
GEOMETRY_TOLERANCE_EMU = 500
...
def reference_icon_hashes():
    """Map each named Align icon in the bundled reference to its artwork hash."""
    path = os.path.join(skill_root(), "assets", "templates",
                        "Align-HCM-Primary-Deck-Reference.pptx")
    ...
    try:
        ...
        return {}
    hashes = {}
    for picture in picture_targets(parts):
        if picture["name"].startswith("ALIGN_ICON_") and picture["media"] in parts:
            hashes[picture["name"]] = sha256(parts[picture["media"]])
    return hashes
...
def validate(args):
    ...
    if geometry is None:
        findings.append(("error", "presentation has no slide size"))
    elif any(abs(actual - expected) > GEOMETRY_TOLERANCE_EMU
             for actual, expected in zip(geometry, EXPECTED_GEOMETRY)):
        findings.append(("error", f"wrong slide geometry: {geometry}; expected "
                                  f"{EXPECTED_GEOMETRY} within {GEOMETRY_TOLERANCE_EMU} EMU"))
    ...
    if not align:
        ...
        for picture in align:
            if picture["media"] not in parts or sha256(parts[picture["media"]]) != expected_align_hash:
                ...
                break
    # Named Align icons must still be the bundled artwork. The expected hashes
    # are read from the reference deck itself rather than pinned in a manifest,
    # so replacing the reference cannot leave a stale list behind.
    icons = [p for p in pictures if p["name"].startswith("ALIGN_ICON_")]
    reference = reference_icon_hashes()
    if reference:
        for picture in icons:
            expected = reference.get(picture["name"])
            if expected is None:
                findings.append(
                    ("warn", f"{picture['name']} is not in the reference deck; "
                             "confirm it is an intentional addition"))
                continue
            actual = parts.get(picture["media"], b"")
            if sha256(actual) != expected:
                findings.append(
                    ("error", f"{picture['name']} artwork does not match the bundled "
                              f"reference (swapped or recolored icon)"))
        missing_icons = sorted(set(reference) - {p["name"] for p in icons})
        if missing_icons:
            findings.append(
                ("warn", f"reference icons absent from this deck: {', '.join(missing_icons)}"))
