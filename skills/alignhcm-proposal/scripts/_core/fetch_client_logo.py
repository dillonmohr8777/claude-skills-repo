...
def evaluate(candidate, target_width, problems=None):
    ...
    if not data or len(data) < 64:
        ...
        return None
    return decode(data, final or url, candidate["kind"], target_width, ctype)


def decode(data, url, kind, target_width, ctype=""):
    """
    Bytes to a scored candidate.

    Split out from `evaluate` so a file supplied by hand goes through exactly
    the same rasterising, decoding, cleanup, and quality gate as one pulled off
    a website. Two paths to a client logo with two different standards is how a
    stretched favicon ends up on a cover.
    """
    ...
    try:
        ...
        return {"url": url, "kind": kind, "error": str(exc)}
...
def from_file(args):
    """Run a supplied file through the cleanup, plate, and quality gate."""
    try:
        with open(args.from_file, "rb") as fh:
            data = fh.read()
    except OSError as exc:
        log(f"error: cannot read {args.from_file}: {exc}")
        return 4
    entry = decode(data, args.from_file, "supplied", args.target_width)
    if "img" not in entry:
        log(f"error: {entry.get('error', 'could not decode')}")
        return 3
    entry["score"] = score(entry)
    log(f"supplied {entry['width']}x{entry['height']} "
        f"alpha={'y' if entry['has_alpha'] else 'n'}")
    return finish(entry, args, source=args.from_file)
...
def finish(best, args, *, host=None, source=None):
    """
    Everything after a candidate is chosen: clean, gate, plate, gate again,
    write the PNG and its provenance sidecar.

    Shared by the fetch path and --from-file so both are held to one standard.
    """
    host = host or ""
    ...
    payload = {
        ...
        "domain": host,
        "source_url": best.get("url", source),
...
def main():
    ...
    ap.add_argument("--from-file",
                    help="clean and plate a logo you already have, instead of "
                         "fetching one. Same cleanup and quality gate")
    ...
    if not args.domain and not args.from_file:
        ap.error("give either --domain or --from-file")
    ...
    return finish(best, args, host=host, source=best["url"])
